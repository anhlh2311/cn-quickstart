...
if [ "$LP_SELF" = "true" ]; then
  ...
  LP_API="${LP_API:-http://localhost:3002}"
  LP_TOKEN="${LP_TOKEN:-${LP_BACKEND_API_KEY:-local-lp-api-key}}"
  LP_BODY=$(jq -n \
    --arg lpPartyId "$LP_PARTY_ID" \
    --arg lpName "$LP_NAME" \
    --arg lpApi "$LP_API" \
...
if [ -z "$LP_BACKEND_DIR" ]; then
  ...
  if [ ! -d "$LP_BACKEND_DIR" ]; then
    ...
    # The LP party is the one allocated/found above — use it directly.
    LP_VALIDATOR_PARTY="$LP_PARTY_ID"
    log "  LIQUIDITY_PROVIDER_PARTY_ID=$LP_VALIDATOR_PARTY"
    ...
    # LP_TOKEN is what the exchange backend sends to the LP backend as x-api-key (stored in lp_token DB column).
    # LP_BACKEND_API_KEY is a legacy alias; LP_TOKEN takes priority.
    # Both must match — API_KEYS is what the LP backend validates incoming requests against.
    LP_BACKEND_API_KEY="${LP_BACKEND_API_KEY:-local-lp-api-key}"
    LP_TOKEN="${LP_TOKEN:-$LP_BACKEND_API_KEY}"
...
# Must match LP_TOKEN stored in exchange backend's liquidity_providers.lp_token column
API_KEYS=$LP_TOKEN
...
LIQUIDITY_PROVIDER_PARTY_ID=$LP_VALIDATOR_PARTY
...
##############################################################################
# Update backend .env with resolved LP party ID
##############################################################################

BACKEND_ENV="$EXCHANGE_BACKEND_DIR/.env"
if [ -f "$BACKEND_ENV" ]; then
  log ""
  log "Updating backend .env with LIQUIDITY_PROVIDER_PARTY_ID..."
  if grep -q "^LIQUIDITY_PROVIDER_PARTY_ID=" "$BACKEND_ENV"; then
    sed -i.bak "s|^LIQUIDITY_PROVIDER_PARTY_ID=.*|LIQUIDITY_PROVIDER_PARTY_ID=${LP_PARTY_ID}|" "$BACKEND_ENV"
  else
    echo "LIQUIDITY_PROVIDER_PARTY_ID=${LP_PARTY_ID}" >> "$BACKEND_ENV"
  fi
  log "  Updated: $BACKEND_ENV"
  log "  Restart the backend for the change to take effect."
fi
