...
#
# Reads holdings from user-wallet-holdings-amulet.json (produced by 08-query-holdings-amulet.sh)
# and uses MergeDelegation_Merge to exercise a self-transfer via ExternalPartyAmuletRules
# (TransferFactory interface). The operator submits via regular submission.
#
...
#   3. Records the merged Amulet
#
# Prerequisites:
#   - quickstart must be running (DevNet mode)
#   - 08-query-holdings-amulet.sh must have been run (creates user-wallet-holdings-amulet.json)
...
#
# Usage: ./10-merge-holdings-amulet.sh
...
if [ ! -f "$BACKEND_ENV" ]; then
  echo "[merge-amulet] ERROR: Backend .env not found: $BACKEND_ENV" >&2
  exit 1
fi
OPERATOR_PARTY=$(grep -E '^EXECUTOR_PARTY_ID=' "$BACKEND_ENV" | cut -d= -f2-)
if [ -z "$OPERATOR_PARTY" ]; then
  echo "[merge-amulet] ERROR: EXECUTOR_PARTY_ID not found in $BACKEND_ENV" >&2
  exit 1
fi
...
HOLDINGS_INPUT_FILE="$SCRIPT_DIR/user-wallet-holdings-amulet.json"

for f in "$KEYPAIRS_FILE" "$DELEGATIONS_FILE" "$HOLDINGS_INPUT_FILE"; do
  if [ ! -f "$f" ]; then
    echo "[merge-amulet] ERROR: Required file not found: $f" >&2
    exit 1
  fi
done
...
if [ -z "$FAR_CID" ] || [ "$FAR_CID" = "null" ]; then
  echo "[merge-amulet] ERROR: FeaturedAppRight contractId not found in $FAR_FILE" >&2
  exit 1
fi
...
log "=========================================="
log "Merge Amulet (CC) Holdings (from JSON)"
log "=========================================="
...
##############################################################################
# Step 2: Merge Amulet holdings for each wallet (from JSON input)
##############################################################################
...
for i in $(seq 0 $((NUM_WALLETS - 1))); do
  ...
  # 2b. Read holdings from JSON file
...
##############################################################################
# Step 4: Verify balances match input holdings
##############################################################################
...
for i in $(seq 0 $((NUM_WALLETS - 1))); do
  ...
  # Compare as integers (truncate decimals) to avoid formatting differences
...
log ""
log "=========================================="
log "Amulet Holdings Merge Complete!"
log "=========================================="
log ""
log "Summary:"
