...
#
# Uses MergeDelegation_Merge to exercise a self-transfer via the Utility package's
# AllocationFactory (TransferFactory interface). The operator (executor party) submits
# the merge via regular submission — no wallet private keys are needed.
#
# Steps:
#   1. Loads AllocationFactory + InstrumentConfiguration from cbtc-factories.json
#   2. Loads FeaturedAppRight from featured-app-right.json
#   3. For each wallet:
#      a. Validates that a MergeDelegation contract exists
#      b. Queries all active Holding contracts
#      c. Exercises MergeDelegation_Merge (operator submits, single regular submission)
#      d. Records the merged Holding
#   4. Writes user-wallet-merged-holdings-cbtc.json
#   5. Verifies balances match the original user-wallet-holdings-cbtc.json
#
# Prerequisites:
...
#
# Usage: ./05-user-wallet-utxo-merging-cbtc.sh
...
if [ ! -f "$BACKEND_ENV" ]; then
  echo "[utxo-merge-cbtc] ERROR: Backend .env not found: $BACKEND_ENV" >&2
  exit 1
fi
OPERATOR_PARTY=$(grep -E '^EXECUTOR_PARTY_ID=' "$BACKEND_ENV" | cut -d= -f2-)
if [ -z "$OPERATOR_PARTY" ]; then
  echo "[utxo-merge-cbtc] ERROR: EXECUTOR_PARTY_ID not found in $BACKEND_ENV" >&2
  exit 1
fi
...
ORIGINAL_HOLDINGS_FILE="$SCRIPT_DIR/user-wallet-holdings-cbtc.json"

for f in "$KEYPAIRS_FILE" "$DELEGATIONS_FILE" "$ORIGINAL_HOLDINGS_FILE"; do
  if [ ! -f "$f" ]; then
    echo "[utxo-merge-cbtc] ERROR: Required file not found: $f" >&2
    exit 1
  fi
done
...
if [ -z "$FACTORY_CID" ] || [ "$FACTORY_CID" = "null" ]; then
  echo "[utxo-merge-cbtc] ERROR: AllocationFactory contractId not found in $FACTORIES_FILE" >&2
  exit 1
fi
if [ -z "$INSTRUMENT_CONFIG_CID" ] || [ "$INSTRUMENT_CONFIG_CID" = "null" ]; then
  echo "[utxo-merge-cbtc] ERROR: InstrumentConfiguration contractId not found in $FACTORIES_FILE" >&2
  exit 1
fi
...
if [ -z "$FAR_CID" ] || [ "$FAR_CID" = "null" ]; then
  echo "[utxo-merge-cbtc] ERROR: FeaturedAppRight contractId not found in $FAR_FILE" >&2
  exit 1
fi
...
log() {
  echo "[utxo-merge-cbtc] $*"
}

log_error() {
  echo "[utxo-merge-cbtc] ERROR: $*" >&2
}
...
# Usage: regular_submit <actAs_party> <readAs_party> <command_json> <cmd_id_prefix> [disclosed_json]
regular_submit() {
  local party="$1"
  local read_as="$2"
...
log "=========================================="
log "UTXO Merging — Merge CBTC Holdings (Utility)"
log "=========================================="
...
##############################################################################
# Step 1: Merge holdings for each wallet
##############################################################################

log ""
log "Step 1: Merging holdings for $NUM_WALLETS wallets..."
...
for i in $(seq 0 $((NUM_WALLETS - 1))); do
  ...
  # 1b. Query all Holding contracts for this wallet (utility Holding)
  ...
  HOLDINGS_DATA=$(echo "$HOLDINGS_RESPONSE" | jq -c '
    [.[] | select(.contractEntry.JsActiveContract) | .contractEntry.JsActiveContract.createdEvent
     | { contractId: .contractId, amount: (.createArguments.amount // .createArgument.amount) }]
  ' 2>/dev/null || echo "[]")
  ...
  MERGE_TX=$(regular_submit "$OPERATOR_PARTY" "$WALLET_PARTY" "$MERGE_CMD" \
    "merge-cbtc-$WALLET_HINT" "$DISCLOSED_CONTRACTS") || {
...
##############################################################################
# Step 3: Verify balances match original holdings
##############################################################################
...
log ""
log "=========================================="
log "CBTC UTXO Merging Complete!"
log "=========================================="
log ""
log "Summary:"
