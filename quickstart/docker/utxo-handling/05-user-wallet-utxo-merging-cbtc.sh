...
#
# Uses the Utility package's AllocationFactory (which implements TransferFactory interface)
# to perform a self-transfer merge: all holdings of a wallet are combined into one.
#
# Steps:
...
#      c. Performs a two-step self-transfer via TransferFactory_Transfer (interface on AllocationFactory)
#      d. Accepts the TransferOffer via TransferInstruction_Accept
#      e. Records the merged Holding
#   3. Writes user-wallet-merged-holdings-cbtc.json
#   4. Verifies balances match the original user-wallet-holdings-cbtc.json
#
# Prerequisites:
#   - quickstart must be running
#   - 03-register-cbtc-token.sh must have been run (creates cbtc-factories.json)
#   - request-minting-cbtc.sh must have been run (creates user-wallet-holdings-cbtc.json)
#   - create-merge-delegation.sh must have been run (MergeDelegation contracts)
#
# Usage: ./user-wallet-utxo-merging-cbtc.sh
...
interactive_submit() {
  ...
  for attempt in 1 2 3; do
    ...
    jq -n \
      --arg userId "$user_id" \
      --arg submissionId "utxo-merge-cbtc-${cmd_id}" \
...
for i in $(seq 0 $((NUM_WALLETS - 1))); do
  ...
  # 1b. Query all Holding contracts for this wallet (utility Holding, not fungible-token)
