...
All resolved values are loaded from `trade-request-config.json` (generated by `03-request-partner-api-key.sh`). Individual JSON files are used as fallbacks for backward compatibility. Only `setup-trading-partner/.env` is sourced — no dependency on `setup-exchange/.env`. All values can be overridden via environment variables:
